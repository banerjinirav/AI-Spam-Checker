...
for word in wordsToCheckList:
    index = words_to_indicies[word]
    total_occurrences = 0
    spamWords = set(messages_to_quantities_spam.keys())
    hamWords = set(messages_to_quantities_ham.keys())
    if word in spamWords:
        total_occurrences += messages_to_quantities_spam[word]
    if word in hamWords:
        total_occurrences += messages_to_quantities_ham[word]
    wordCount = 0
    for i in range(len(list_of_vectors)):
        if(list_of_vectors[i][index] == 1):
            wordCount += 1
    if(wordCount != total_occurrences):
        print("ERROR")
        print(word)
        print(wordCount)
